...
def mostrar_menu_volumen(font: pygame.font.Font, small_font: pygame.font.Font, screen: pygame.Surface, cambiar_fondo, OPCIONES_IMAGE: pygame.Surface, reproducir_sonido, sonidos: dict, estado_config: dict, guardar_config, GRAY: tuple, BLACK: tuple, WIDTH: int, HEIGHT: int) -> None:
    """
    ...
    while activo:
        cambiar_fondo(r"Terrile-Kioso-Aritmetica\modulos\assets\fondo_menu.png")
        ...
        _, indicador_music = dibujar_slider("Música", slider_music_pos[0], slider_music_pos[1], slider_width, slider_height, estado_config["music_volume"], small_font, BLACK, GRAY, RED, screen)
        _, indicador_sounds = dibujar_slider("Sonidos", slider_sounds_pos[0], slider_sounds_pos[1], slider_width, slider_height, estado_config["sounds_volume"], small_font, BLACK, GRAY, RED, screen)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:  # Verifica si el botón izquierdo del ratón fue presionado
                    mouse_pos = event.pos
                    if indicador_music.collidepoint(mouse_pos):
                        slider_music = True
                    elif indicador_sounds.collidepoint(mouse_pos):
                        slider_sounds = True
                    elif volver_button_rect.collidepoint(mouse_pos):  # Botón "Volver al Menú"
